...
class States(BaseClass):

    def __init__(self):
        super().__init__()
        self.file_name = path.abspath(path.join(__file__, '../../data/states.xls'))
    ...
    def read_data_from_file(self):
        ...
        if workbook:
            ...
            for y in range(sheet.nrows):  # y for rows
                ...
                if len(cell_code) == 2 and cell_name:
                    doc = {
                        'code': cell_code,
                        'name': cell_name,
                    ...
                    self.mongo.state.update({'code': doc['code']}, doc, True)
